...
// URL Normalizer Middleware for Vercel / Serverless Environments
app.use((req: any, _res: any, next: any) => {
    if (req.url === "/api" || req.url === "/api/" || req.url === "/api/index.ts") {
...
async function getCashfreeConfig() {
    ...
    if (!appId || !secretKey) {
        console.warn("WARNING: Cashfree API keys missing in environment variables and Firebase DB.");
    }
...
// Handle missing /api routes with JSON instead of Express default HTML 404
...
async function startServer() {
    if (process.env.VERCEL) {
        // On Vercel, static files are served by Vercel CDN; Express only handles API requests.
        return;
    }
